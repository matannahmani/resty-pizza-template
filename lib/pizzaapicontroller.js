...
const apipostProduct = async (props) => 
    await axios.post(`${process.env.API_URL}/shop/${process.env.SHOP_ID}/products`, {product: props})
      .then((response) =>{
          return {
              code: 200,
              data: response.data
...
const apipatchProduct = async (props) =>
    await axios.patch(`${process.env.API_URL}/shop/${process.env.SHOP_ID}/products/${props.id}`, {product: (({ id,product, ...o }) => o)(props)})
        .then((response) =>{
            return {
                code: 200,
                data: response.data
...
const apideleteProduct = (props) => {
    axios.delete(`${process.env.API_URL}/shop/${process.env.SHOP_ID}/products/${props.id}`)
...
await axios.get(`${process.env.API_URL}/shop/${process.env.SHOP_ID}/products`)
    .then(res => ({
        code: 200,
        data: res.data,
    }))
