...
const apipatchOrder = async (props) => 
    await axios.patch(`${process.env.API_URL}/orders/${props.id}`, {order: {status: props.status,done: props.done}})
      .then(res => ({
        code: 200,
        data: res.data,
    }))
...
export {apipostOrder,apigetOrder,apipatchOrder,apicheckOrder,apicheckCart};
